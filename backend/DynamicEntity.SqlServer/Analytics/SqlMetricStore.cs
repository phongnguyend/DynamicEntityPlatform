...
public sealed class SqlMetricStore(SqlServerOptions options) : IMetricStore
{
    ...
    private async Task<SqlConnection> OpenAsync(EntityStorageLocation storage,CancellationToken token){var c=SqlServerTenantConnection.Create(options,storage);await c.OpenAsync(token);return c;}
