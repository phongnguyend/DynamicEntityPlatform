...
public sealed class SqlMetricStore : IMetricStore
{
    ...
    private async Task<SqlConnection> OpenAsync(EntityStorageLocation storage,CancellationToken token){var c=SqlServerTenantConnection.Create(storage);await c.OpenAsync(token);return c;}
