...
export function DashboardsListPage({ tenantId }: { tenantId: string }) {
  ...
  return <section className="page"><header><div><p className="eyebrow">Workspace overview</p><h2>Dashboards</h2></div></header>
    <div className="panel"><div className="panel-header"><h3>Dashboards</h3><div className="actions"><span>{dashboards.data?.length ?? 0} dashboard{dashboards.data?.length === 1 ? '' : 's'}</span><Link className="button" to="/dashboards/new"><Plus />New dashboard</Link></div></div>
